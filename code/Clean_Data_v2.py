...
def combine_stints(df, player_pos):
    df_combined_stints = df.groupby(['playerID', 'yearID']).sum().reset_index().sort_values(by=['playerID', 'yearID'])
    return df_combined_stints.merge(player_pos, on='playerID')
...
def get_cumulative_stats(df):
    ...
    df_cumulative_stats = df.groupby('playerID')[stats].cumsum()
    cols = list(set(df.columns) - set(df_cumulative_stats.columns))
    return df_cumulative_stats.join(df[cols])[df.columns]
